...
// 拉取上游支持的模型列表（OpenAI 兼容 GET /models 端点）
//   upstream: {host, port, pathPrefix, useProxy, customProxyUrl, apiKey}
//   返回 Promise<{models:[{name,contextK}], raw}>；失败 reject(Error)
// 路径策略：先试 pathPrefix + /models；404 再试 pathPrefix + /v1/models（兼容差异）
function fetchModels(upstream) {
  ...
  return tryPath(pathPrefix + "/models")
    .then((res) => {
      if (res.status === 404 && !pathPrefix.endsWith("/v1")) {
        // 回退：试 /v1/models（针对 pathPrefix 不含 /v1 的上游）
