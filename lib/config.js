...
// [{id, name, host, port, pathPrefix, useProxy, customProxyUrl, apiKey, models:[{name, contextK?}]}]
// apiKey 绑在上游上（不再在路由里）；models 可由「拉取」按钮自动填充或手填
...
function setUpstreams(list) {
  ...
  const clean = list.map((u) => ({
    ...
    customProxyUrl: typeof u.customProxyUrl === "string" ? u.customProxyUrl.trim() : "",
...
// [{id, clientModel, upstreams:[{upstreamId, targetModel}, ...]}]
